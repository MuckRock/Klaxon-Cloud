...
- `Header.svelte` — top bar with the Sign in/Sign out button and (when authenticated) the navigation between alert views. Reads `authState` directly. No close button — the browser owns the panel chrome. The right edge holds the shared `HelpMenu` (`@klaxon/lib/components/HelpMenu.svelte`, also used by the site header): User Guide → FAQ → a prefilled `mailto:` to support. Links are handed to `chrome.tabs.create` rather than followed, so a click can't navigate the panel away from the app; the mailto body carries the userinfo account details plus the manifest version and user agent.
